...
//Original source from https://github.com/MrMarble/PM2-Telegram-bot/blob/master/src/pm2.js
//Made the file TS instead
...
const restartAsync = (process: string | number) => {
  return new Promise((resolve, reject) => {
...
const describeAsync = (process: string | number) => {
  return new Promise((resolve, reject) => {
...
export async function restart(process: string | number) {
  try {
    ...
    //@ts-ignore Make sure pm2 bot does not get restarted
 ...
 * @returns {Promise<restartResponse>}
 */
export async function describe(process: string | number) {
  try {
    await connectAsync();
